...
public class ExponentialSmoother {
    ...
    public ExponentialSmoother(double smoothingFactor) { this.smoothingFactor = smoothingFactor; }
    ...
    public double update(double newValue) {
        if (!hasUpdated) {
            average = newValue;
            hasUpdated = true;
        } else { average = smoothingFactor * newValue + (1 - smoothingFactor) * average; }
        return average;
    }

    public void reset() { hasUpdated = false; }
