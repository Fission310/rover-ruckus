    ...
    public Pose2d transform(Pose2d original) {
        ...
        switch (mode) {
            ...
            case EXPONENTIAL:
                ...
                break;
            case DUAL_ZONE:
                if (r < dualThreshold) r = dualSlope * r;
                else r = ((1 - dualSlope * dualThreshold) / (1 - dualThreshold)) * (r - 1) + 1;
                if (omega < dualThreshold) omega = dualSlope * omega;
                else omega = ((1 - dualSlope * dualThreshold) / (1 - dualThreshold)) * (omega - 1) + 1;
        }
