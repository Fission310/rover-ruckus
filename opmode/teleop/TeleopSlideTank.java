...
//        slideInput = abs(gamepad1.left_stick_x) > .9 ? Math.signum(gamepad1.left_stick_x) : .8 * gamepad1.left_stick_x;
        ...
        else leftBumperPressed = false;
        telemetry.addData("Status", "left bumper" + leftBumper);

        if (leftBumper) { robot.acquirer.acquirerRotationAcquirer(); }
...
//        robot.acquirer.setAcquirerRotation(acquirerRotation);
         ...
         * Telemetry
         */
        telemetry.addData("Front Range Sensors",  robot.sensors.frontRange.getRangeRawUltrasonic());
        telemetry.addData("Back Range Sensors",  robot.sensors.backRange.getRangeRawUltrasonic());
